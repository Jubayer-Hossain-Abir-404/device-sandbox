...
trait FilterTrait
{
    public function scopeFilter(Builder $query, array $filters = []): Builder
    {
        if ($filters) {
            ...
            foreach ($filters as $field => $value) {
                if (
                    (!in_array($field, $defaultFillableFields, true) || !$value)
                        && $value != '0'
                ) {
                    continue;
                }
    ...
    public function scopeWithFilter(Builder $query, string $relationName, array $filters = []): Builder
    {
        try {
            ...
            throw new ModelNotFoundException($e->getMessage(), Res::HTTP_NOT_FOUND);
        }
    ...
    public function scopeSearch(Builder $query, array $filters = []): Builder
    {
        if (array_key_exists('s', $filters)) {
            ...
            return $this->buildSearchQuery($query, $term);
        }
